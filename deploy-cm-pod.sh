#!/bin/bash
# Copyright (c) 2018, TIBCO Software Inc. All rights reserved.
#
ACTION=create

ZONE=$1
if [ "$ZONE" != "" ]; then
    TML_DEPLOY_FILE=$(grep -inl "value: \"$ZONE\"" cm-pod-*.yaml)
    if [ "$TML_DEPLOY_FILE" != "" ]; then
        echo "Deployment file $TML_DEPLOY_FILE found for the given zone $ZONE. Deploying for the zone."
    	kubectl $ACTION -f $TML_DEPLOY_FILE
    	status=$(kubectl get secrets | grep -w cm-property)
    	if [ $? -ne 0 ]; then
        	kubectl $ACTION secret generic cm-property --from-file=tml_cm_properties.json
			kubectl $ACTION secret generic cm-jks --from-file=tml-cm.jks
			kubectl $ACTION secret generic cm-crt --from-file=tml-cm-crt.pem
			kubectl $ACTION secret generic cm-key --from-file=tml-cm-key.pk8
            kubectl $ACTION secret generic cm-resource --from-file=resources/tml-cm/
    	fi
    	exit 0;
    else
    	echo "No deployment file found for the given zone. Please enter a valid zone."
    	exit 1
    fi
fi

kubectl $ACTION secret generic cm-property --from-file=tml_cm_properties.json
kubectl $ACTION secret generic cm-jks --from-file=tml-cm.jks
kubectl $ACTION secret generic cm-crt --from-file=tml-cm-crt.pem
kubectl $ACTION secret generic cm-key --from-file=tml-cm-key.pk8
kubectl $ACTION secret generic cm-resource --from-file=resources/tml-cm/

for index in $(seq 0 0); do
	cm_pod_yaml="cm-pod-${index}.yaml"
	kubectl $ACTION -f $cm_pod_yaml
done
#Provide some time for pod's creation
sleep 10

# Check cm pods deployment progress
while [ : ]
do
    status=$(kubectl get pods| grep "cm-deploy")
    if [ $? -ne 0 ]; then
        break
    fi
    
    echo
    echo "$status"
    count=$(echo "$status" | grep "1/1 *Running" | wc -l)
    if [ $? -eq 0 ] && [ $count -eq $((1 * 1)) ]; then
        break
    fi
    sleep 5
done
